...
def render_block(agent_name: str, agent) -> str:
    ...
    for nombre in acciones:
        ...
        lineas.append(f"| `run {agent_name} {nombre}`{marca} | {_signature_args(acciones[nombre])} |")
    if not acciones:
    ...
    if contrato is not None:
        ...
        if destructivas:
            lineas.append("**No se deshacen** (la puerta de permisos las bloquea sin `--yes`; propón, no ejecutes): " + ", ".join(f"`{a}`" for a in destructivas))
            lineas.append("")
        if contrato.cannot:
...
def render_claude_agent(nombre: str, cuerpo: str) -> str:
    ...
    lineas += [
        "---",
        "",
        "<!-- Generado desde .opencode/agents/" + nombre + ".md por `make assistants-sync`. Edita el original, no este fichero. -->",
        "",
        cuerpo.strip(),
        "",
    ]

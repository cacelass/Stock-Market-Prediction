...
| `run git commit_feature` | `--id`, `--title`, `--message`, `--dry_run` |
| `run git tag_release` | `--version` (obligatorio) · `--message`, `--since_tag` |
| `run git create_branch` | `--branch_name` (obligatorio) · `--base_branch` |
| `run git merge_branch` | `--source_branch` (obligatorio) · `--target_branch` |

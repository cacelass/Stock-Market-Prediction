...
_COMANDOS_BLOQUEADOS: tuple[tuple[str, str], ...] = (
    (r"\brm\s+(-[a-zA-Z]*\s+)*-?[a-zA-Z]*[rf][a-zA-Z]*\s+(/|~|\$HOME)(\s|$)", "borrado recursivo fuera del proyecto"),
    (r"\bgit\s+push\b", "el push es siempre una decisión del humano (ver AGENTS.md)"),
    (r"\bgit\s+(reset\s+--hard|clean\s+-[a-zA-Z]*f)", "descarta trabajo sin posibilidad de recuperarlo"),
    (r"\bgit\s+(push\s+.*--force|.*\s--force-with-lease)", "reescribe historial publicado"),
    (r"\b(curl|wget)\b[^|]*\|\s*(sudo\s+)?(ba)?sh", "descargar y ejecutar en un solo paso: no hay forma de revisar qué se ejecuta"),
    (r"\bsudo\b", "elevar privilegios queda fuera de lo que un agente decide"),
    (r"\bmkfs(\.|\s)|\bdd\s+if=.*of=/dev/", "escritura directa sobre dispositivos"),
    (r"\bchmod\s+(-R\s+)?777\b", "permisos abiertos a todo el mundo"),
    (r":\(\)\s*\{.*\};\s*:", "fork bomb"),
    (r"\bhistory\s+-c\b|>\s*~/\.bash_history", "borrar el rastro de lo ejecutado"),
)
...
def evaluar(evento: dict, raiz: Path | None = None) -> tuple[int, str]:
    ...
    if herramienta in _HERRAMIENTAS_DE_FICHERO:
        ...
        if motivo:
            return BLOQUEAR, (f"Bloqueado por la política del proyecto: {motivo}. Si necesitas su contenido, pide al humano el valor concreto — no leas el fichero.")
        if herramienta != "Read" and _fuera_del_proyecto(ruta, raiz):
            return BLOQUEAR, (f"Bloqueado: '{ruta}' queda fuera de la raíz del proyecto ({raiz}). Los agentes escriben dentro del proyecto.")
    ...
    if herramienta.startswith("mcp__"):
        for valor in _valores_de_texto(entrada):
            ...
            if motivo:
                return BLOQUEAR, (f"Bloqueado por la política del proyecto: {motivo}. La llamada a '{herramienta}' referencia '{valor}'.")

    if herramienta in _HERRAMIENTAS_DE_SHELL:
        comando = str(entrada.get("command") or "")
        motivo = _revisar_comando(comando)
        if motivo:
            return BLOQUEAR, (f"Bloqueado por la política del proyecto: {motivo}. Comando: {comando[:200]}")
        ...
        for token in re.findall(r"[\w./~-]+", comando):
            ...
            if prohibido:
                return BLOQUEAR, (f"Bloqueado por la política del proyecto: {prohibido}. El comando referencia '{token}'.")

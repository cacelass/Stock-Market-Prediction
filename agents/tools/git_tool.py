...
class GitTool:
    ...
    def log(self, *, max_count: int = 20, since_tag: str | None = None) -> list[dict[str, str]]:
        ...
        result = self._git("log", rev_range, f"--max-count={max_count}", f"--pretty=format:{fmt}", "--date=short")
        commits = []
        for line in result.stdout.splitlines():
            if not line.strip():
                continue
    ...
    def plan_atomic(changed_files: list[str], repo_root: Path) -> dict:
        ...
        for f in files:
            ...
            group = by_area.setdefault(area, {"area": area, "type": commit_type, "priority": priority, "files": []})

...
def _idf_desde_postings(postings: dict[str, list[tuple[int, int]]], n: int) -> dict[str, float]:
    return {t: math.log(1 + (n - len(p) + 0.5) / (len(p) + 0.5)) for t, p in postings.items()}
...
class RagTool:
    ...
    @staticmethod
    def _collection(client: Any, embedder: str | None = None) -> Any:
        """
    ...
    @staticmethod
    def _add_chunk(chunks: list[Any], text: str, source: str, line: int, section_type: str = "fallback", file_hash: str = "") -> None:
        """
    ...
    @staticmethod
    def _ensure_not_empty(chunks: list[Any], content: str, source: str, section_type: str = "fallback", file_hash: str = "") -> list[Any]:
        """
    ...
    @staticmethod
    def _make_chunk(text: str, source: str, line: int, section_type: str = "fallback", file_hash: str = "") -> dict[str, Any] | None:
    ...
    def chunk_py(content: str, source: str, file_hash: str = "") -> list[dict[str, Any]]:
        ...
        for i, line in enumerate(lines):
            ...
            if match_class:
                ...
                tipo = "function"
                if match_def is not None and not match_def.group(1):
    ...
    def chunk_md(content: str, source: str, file_hash: str = "") -> list[dict[str, Any]]:
        ...
        def _cerrar() -> None:
            ...
            RagTool._add_chunk(
                ...
                source,
                inicio,
                "heading" if ruta else "paragraph",
                file_hash,
            )
        ...
        if not chunks:
            chunks = RagTool._chunk_by_size(content, source, file_hash)
        return RagTool._ensure_not_empty(chunks, content, source, "heading" if ruta else "paragraph", file_hash)
    ...
    def _chunk_by_size(content: str, source: str, file_hash: str = "") -> list[dict[str, Any]]:
        ...
        def _volcar() -> None:
            if acumulado:
                RagTool._add_chunk(chunks, " ".join(acumulado), source, linea_inicio, "fallback", file_hash)
    ...
    def _trocear(origen: str, texto: str, tipo: str, huella: str) -> list[dict[str, Any]]:
        return RagTool.chunk_py(texto, origen, huella) if tipo == "py" else RagTool.chunk_md(texto, origen, huella)
    ...
    def index_project(root: Path, rebuild: bool = False) -> dict[str, Any]:
        ...
        huerfanos = [cid for origen, cids in ids_por_origen.items() if origen not in vivos and not origen.startswith("url:") for cid in cids]
        if huerfanos:
    ...
    @staticmethod
    def _completar_similitudes(collection: Any, query: str, ids: list[str], similitudes: dict[str, float]) -> None:
        """
        ...
        try:
            ...
            for cid, vector in zip(datos["ids"], datos["embeddings"] or [], strict=False):
                similitudes[cid] = round(RagTool._coseno(vector_consulta, list(vector)), 4)
    ...
    def _expandir(collection: Any, resultados: list[dict[str, Any]], vecinos: int) -> None:
        ...
        for fuente in por_fuente:
            ...
            trozos = [
                ...
                for cid, doc, meta in zip(datos["ids"], datos["documents"] or [], datos["metadatas"] or [], strict=False)
            ]
    ...
    def search(
        ...
        *,
        ...
        textos: dict[str, str] = {}
        metadatos: dict[str, dict[str, Any]] = {}
            ...
            salida.append(
                {
                    ...
                    "match": ("ambos" if cid in en_vector and cid in en_lexico else "lexico" if cid in en_lexico else "vector"),
                }
            )
            if len(salida) >= top_k:
                break
    ...
    def status(root: Path) -> dict[str, Any]:
        ...
        desfasados = sorted(o for o, h in vivos.items() if o in huella_por_origen and huella_por_origen[o] != h)
        ...
        borrados = sorted(o for o in huella_por_origen if o not in vivos and not o.startswith("url:"))

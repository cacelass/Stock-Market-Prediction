...
class KnowledgeTool:
            ...
            papers.append(
                {
                    ...
                    "url": url,
                    "authors": [(a.findtext("a:name", default="", namespaces=_ATOM) or "").strip() for a in entry.findall("a:author", _ATOM)],
                }
            )
        return papers
    ...
    def _pdf_a_markdown(pdf_bytes: bytes) -> str:
        ...
        try:
            ...
            raise KnowledgeError("markitdown no está instalado y el paper no tiene HTML de arXiv. Ejecuta: uv sync --extra rag") from exc
    ...
    def refresh(
        ...
        *,
        ...
        for tema in data.get("topics", []):
                    ...
                    informe["updated_sources"].append(
                        {
                            ...
                            "desde": src.get("version"),
                            "hasta": actual["version"],
                        }
                    )
                else:
            ...
            informe["new_papers"].extend({"topic": tema.get("topic"), **p} for p in nuevas[:max_new])
            informe["topics"].append(bloque)

        if not dry_run:
            destino = root / "docs" / "knowledge" / "papers"
            for tema in informe["topics"]:
                entradas = next((t for t in data.get("topics", []) if t.get("topic") == tema["topic"]), None)
                if entradas is None:
                    continue

...
CONTRACTS: dict[str, Contract] = {
    ...
    "sentiment": Contract(
        role="Analista de sentimiento financiero por ticker: descarga noticias y puntúa con VADER.",
        can=(
            "descargar noticias RSS de un ticker a data/raw/news_<TICKER>.csv (fetch)",
            "puntuar noticias con VADER + léxico financiero y agregar scores diarios (analyze)",
        ),
        cannot=(
            "modificar datasets de precios ni entrenar modelos → data / ml",
            "escribir fuera de data/raw/news_*.csv y de su workspace",
        ),
        needs=("ticker del activo (p.ej. NVDA)",),
        collaborates=(),
    ),
    "ml": Contract(

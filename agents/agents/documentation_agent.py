...
class DocumentationAgent(BaseAgent):
    ...
    def check_readme_makefile_sync(self) -> AgentResult:
        ...
        targets = {t for t in _MAKE_TARGET_RE.findall(makefile_text) if t not in _INTERNAL_TARGETS and not t.startswith(".")}
        ...
        return AgentResult(
            True,
            self.name,
            "check_readme_makefile_sync",
            f"{len(targets)} targets en Makefile, {len(undocumented)} sin mencionar en README.md.",
            data={"all_targets": sorted(targets), "undocumented": undocumented},
            warnings=warnings,
        )

    def update_changelog(self, *, since_tag: str | None = None, dry_run: bool = True, feature_id: str = "", feature_title: str = "") -> AgentResult:
        """
        ...
        if feature_id and feature_title:
            entry = f"## [Unreleased] — {date.today().isoformat()}\n\n### Añadido\n\n- {feature_title} ({feature_id})\n"
            if dry_run:
                return AgentResult(
                    True,
                    self.name,
        ...
        if dry_run:
            return AgentResult(
                ...
                self.name,
                "update_changelog",
                "Entrada generada en modo dry_run (no se escribió en disco). Vuelve a llamar con dry_run=False para aplicarla.",
                data=entry,
            )
    ...
    def update_prd(self, *, dry_run: bool = False) -> AgentResult:
        ...
        if dry_run:
            return AgentResult(
                ...
                data={"markdown": prd, "path": "docs/prd.md"},
                warnings=warnings,
            )
        ...
        return AgentResult(
            ...
            data={"path": str(target.relative_to(self.ctx.root))},
            warnings=warnings,
        )
    ...
    def _backlog_section(self) -> dict:
        ...
        if por_hacer:
            lines.append("**Pendiente de cerrar:** " + ", ".join(f.get("id", "?") for f in por_hacer))
            lines.append("")
    ...
    def bump_version(self, *, new_version: str) -> AgentResult:
        ...
        if pyproject_path.exists():
            ...
            new_text, n = re.subn(r'^version = "[^"]*"', f'version = "{new_version}"', text, count=1, flags=re.MULTILINE)
            if n:
        ...
        return AgentResult(
            ...
            data={"new_version": new_version, "changed_files": changed_files},
            warnings=warnings,
        )

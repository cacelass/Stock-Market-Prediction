...
_BACKLOG_DIRECCION: tuple[dict, ...] = (
    {
        "id": "SCOPE-001",
        "title": "Definir qué se quiere resolver",
        ...
        "criteria": [
            "references/00-objetivo.md con los apartados del spec",
    ...
    {
        "id": "RESEARCH-001",
        "title": "Qué se sabe ya sobre este problema",
        ...
        "criteria": ["references/01-estado-del-arte.md resume cada fuente en 2-3 líneas", "Queda anotado el rango de resultados que reporta la literatura"],
        "depends_on": ["SCOPE-001"],
    },
    {
        "id": "EDA-001",
        "title": "Exploración inicial en los notebooks",
        ...
        "criteria": ["Los notebooks 0-0, 0-1 y 0-2 se ejecutan sin errores", "references/02-eda.md responde si los datos pueden contestar la pregunta"],
        "depends_on": ["RESEARCH-001"],
    },
    {
        "id": "DATA-001",
        "title": "EDA del dataset principal",
    ...
    {
        "id": "FEAT-001",
        "title": "Pipeline de features reproducible",
    ...
    {
        "id": "MODEL-001",
        "title": "Baseline entrenado y evaluado",
...
_RIESGOS_HEURISTICA: tuple[tuple[str, tuple[str, ...]], ...] = (
    (r"\b(login|auth|autentic|usuario|contraseña|password|sesión|session|token)\b", ("sql injection", "fuga de credenciales", "enumeración de usuarios")),
    (r"\b(pago|tarjeta|card|cobro|factura|invoice|transacción|dinero)\b", ("fraude", "doble cobro", "cumplimiento PCI")),
    (r"\b(datos personales|email|correo|dni|teléfono|telefono|dirección|pii)\b", ("privacidad (GDPR)", "fuga de datos")),
    (r"\b(upload|subida|adjunto|fichero|archivo|file)\b", ("path traversal", "subida de malware")),
    (r"\b(api pública|endpoint|api publica|rest)\b", ("abuso de rate-limit", "exposición de datos")),
    (r"\b(dataset|conjunto de datos|features|entrenamiento|train)\b", ("sesgo de datos", "fuga de datos de entrenamiento")),
    (r"\b(modelo|model|serving|inferencia|despliegue|deploy)\b", ("drift y degradación silenciosa", "fallo de monitoreo")),
)
...
class PlanAgent(BaseAgent):
    ...
    def _save_order(self, order: dict) -> None:
        self._order_path(order["id"]).write_text(json.dumps(order, indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    def _questions(order: dict) -> list[str]:
        ...
        for fragment in order.get("unmatched", []):
            questions.append(
                f"No sé qué agente debe hacer: '{fragment}'. Añade el paso a mano en {order.get('_file', 'la orden JSON')} (agente + acción), o quítalo del encargo."
            )
        return questions
    ...
    def _save_scope(self, scope: dict) -> None:
        ...
        self._scope_path().write_text(json.dumps(scope, indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    def _scope_questions(self, scope: dict) -> list[str]:
        ...
        return [f"{clave}: {pregunta}" for clave, pregunta, _req in _SPEC_FIELDS if not answered.get(clave)]
    ...
    def _sembrar_backlog(self, scope: dict) -> list[str]:
        ...
        if propuestas:
            ...
            existentes = {f["id"] for f in doc.get("features", [])} if doc is not None else set()
            siguiente_feat = 1
        ...
        aceptados = [r for r, decision in scope.get("decisiones_riesgo", {}).items() if decision == "aceptar"]
    ...
    def scope(self, reset: bool = False) -> AgentResult:
        ...
        if not questions:
            return AgentResult(
                ...
                self.name,
                "scope",
                "Entrevista completa. Revisa el borrador y ejecuta `run plan scope_commit` para escribir el spec y sembrar el backlog.",
                data=scope,
                needs=[],
            )
        mensaje = f"Entrevista de arranque: faltan {len(questions)} respuesta(s). Responde con `run plan scope_answer` (una o varias a la vez)."
    ...
    def scope_answer(self, **answers) -> AgentResult:
        ...
        for clave, valor in answers.items():
            ...
            if clave not in {k for k, _, _ in _SPEC_FIELDS}:
                return AgentResult(
                    ...
                    self.name,
                    "scope_answer",
                    f"Clave desconocida: '{clave}'. Válidas: {[k for k, _, _ in _SPEC_FIELDS]} + 'features' + 'aceptar_riesgos' + 'descartar_riesgos'.",
                    data=scope,
                    needs=self._scope_questions(scope),
                )
            if clave == "metrica" and not self._metrica_valida(valor):
                return AgentResult(
                    ...
                    self.name,
                    "scope_answer",
                    "La métrica de éxito debe ser un número con umbral (p. ej. 'F1 macro >= 0.80 en validación'). 'Que funcione bien' no pasa.",
                    data=scope,
                    needs=self._scope_questions(scope),
                )
            a[clave] = valor
        ...
        mensaje = (f"Aplicado: {', '.join(aplicado)}. " if aplicado else "Nada que aplicar. ") + (
            f"Quedan {len(pending)} pregunta(s)." if pending else "Entrevista completa — ejecuta `run plan scope_commit`."
        )
        return AgentResult(bool(aplicado), self.name, "scope_answer", mensaje, data=scope, needs=pending)
    ...
    def _pendientes_riesgo(self, scope: dict) -> list[str]:
        ...
        texto = " ".join(str(respuestas.get(c, "")) for c in ("pregunta", "alcance", "datos", "usuarios")) + " " + " ".join(scope.get("features", []))
        detectados = _detectar_riesgos(texto)

        declarados = {r.strip().lower() for r in str(respuestas.get("riesgos", "")).split(";") if r.strip()}
    ...
    def scope_commit(self) -> AgentResult:
        ...
        if pendientes_riesgo:
            return AgentResult(
                ...
                self.name,
                "scope_commit",
                "La heurística detectó riesgos sin decidir: " + ", ".join(pendientes_riesgo) + ". Decide cada uno antes de sembrar: "
                '`scope_answer aceptar_riesgos="..."` o `descartar_riesgos="..."`.',
                data={"pendientes_riesgo": pendientes_riesgo},
                needs=[
                    f'Riesgo detectado: {r}. Responde `scope_answer aceptar_riesgos="{r}"` para sembrarlo como RISK ticket, o `descartar_riesgos="{r}"`.'
        ...
        return AgentResult(True, self.name, "scope_commit", mensaje, data={"objetivo": str(objetivo.relative_to(self.ctx.root)), "sembradas": sembrados})
    ...
    def intake(self, brief: str) -> AgentResult:
        ...
        plan_lines = [f"  [{i}] {s['agent']}.{s['action']}  (confianza {s['confidence']}) ← '{s['fragment']}'" for i, s in enumerate(steps)]
        summary = (
    ...
    def answer(self, order: str, **answers) -> AgentResult:
        ...
        message = (f"Aplicado: {', '.join(applied)}. " if applied else "Ninguna respuesta coincidió con lo pedido. ") + (
            f"Quedan {len(pending)} pregunta(s)." if pending else f"Todo respondido — ejecuta con: run plan execute --order {order}"
        )
    ...
    def execute(self, order: str, auto_commit: bool = False) -> AgentResult:
        ...
        wo["results"] = [{"agent": r.agent, "action": r.action, "success": r.success, "message": r.message} for r in result.results]
        self._save_order(wo)

        to_verify = [f"  [{i}] {r.agent}.{r.action}: {r.message}" for i, r in enumerate(result.results)]
        message = (
    ...
    def status(self, order: str | None = None) -> AgentResult:
        ...
        if order is not None:
            ...
            return AgentResult(
                ...
                self.name,
                "status",
                f"Orden {order}: {wo['status']} ({len(wo['steps'])} pasos, {len(self._questions(wo))} preguntas pendientes)",
                data=wo,
                needs=self._questions(wo),
            )

...
def test_scope_commit_escribe_spec_y_siembra_backlog(context, project_root):
    ...
    (context.root / "harness" / "featureslist.json").write_text('{"version": 1, "project": "T", "features": []}', encoding="utf-8")
    plan = PlanAgent(context=context)
    plan.scope(reset=True)
...
def test_scope_features_se_deduprican(context):
    plan = PlanAgent(context=context)
    plan.scope(reset=True)
    plan.scope_answer(pregunta="Q", metrica="F1 >= 0.8", datos="d", parada="p", features="API-001; MON-001")
...
def test_scope_commit_rehusa_si_riesgos_detectados_sin_decidir(context):
    ...
    plan = PlanAgent(context=context)
    plan.scope(reset=True)
    plan.scope_answer(pregunta="¿Cómo autenticar usuarios en el login?", metrica="tasa >= 0.99", datos="tabla de usuarios", parada="p")
...
def test_scope_riesgos_declarados_no_se_preguntan_de_nuevo(context):
    ...
    plan = PlanAgent(context=context)
    plan.scope(reset=True)
    plan.scope_answer(pregunta="login con contraseña", metrica="tasa >= 0.99", datos="d", parada="p", riesgos="sql injection")

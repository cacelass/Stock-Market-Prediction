...
def test_agregar_sin_resultados_devueltos_no_divide_por_cero():
    casos = [{"success": False, "recall": 0.0, "reciprocal_rank": 0.0, "lexical_hits": 0, "returned": 0}]
...
def test_sin_poder_medir_la_suite_sale_en_verde(tmp_path, monkeypatch):
    monkeypatch.setattr(rag_eval, "evaluate", lambda *a, **k: {"available": False, "reason": "índice vacío"})
...
def test_por_debajo_del_umbral_la_suite_falla(tmp_path, monkeypatch):
    informe = {
        ...
        "hybrid": {"cases": 2, "hit_rate": 0.0, "recall_at_k": 0.0, "mrr": 0.0, "lexical_share": 0.0, "top_k": 5},
        "vector_only": {"cases": 2, "hit_rate": 0.0, "recall_at_k": 0.0, "mrr": 0.0, "lexical_share": 0.0, "top_k": 5},
        "cases": [
...
def test_por_encima_del_umbral_los_fallos_sueltos_no_tinen_la_suite(tmp_path, monkeypatch):
    ...
    informe = {
        "available": True,
        "index_up_to_date": True,
        "hybrid": {"cases": 4, "hit_rate": 0.75, "recall_at_k": 0.75, "mrr": 0.6, "lexical_share": 0.5, "top_k": 5},
        "vector_only": {"cases": 4, "hit_rate": 0.5, "recall_at_k": 0.5, "mrr": 0.4, "lexical_share": 0.0, "top_k": 5},
        "cases": [

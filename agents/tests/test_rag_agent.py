...
requiere_chroma = pytest.mark.skipif(not RagTool.available(), reason="chromadb no instalado (uv sync --extra rag)")
...
class TestEmbedder:
    ...
    def test_desajuste_detectado(self, monkeypatch):
        coleccion = type("Col", (), {"metadata": {"embedder": "multilingual"}, "count": lambda self: 10})()
    ...
    def test_indice_vacio_no_es_desajuste(self, monkeypatch):
        coleccion = type("Col", (), {"metadata": {"embedder": "multilingual"}, "count": lambda self: 0})()
        monkeypatch.setenv("DSKIT_RAG_EMBEDDER", "onnx")

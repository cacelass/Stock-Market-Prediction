        ...
        "topics": [
            {
                ...
                "sources": [
                    {"arxiv_id": "1412.6980", "title": "Adam", "version": version, "status": "activa", "last_checked": "2026-01-01"},
                ],
            }
        ],
    }
    (tmp_path / "docs" / "knowledge").mkdir(parents=True, exist_ok=True)
    (tmp_path / "docs" / "knowledge" / "sources.json").write_text(json.dumps(data, ensure_ascii=False), encoding="utf-8")
    return data
...
def test_sin_html_y_sin_markitdown_falla_con_instruccion(tmp_path, monkeypatch):
    monkeypatch.setattr(KnowledgeTool, "_html_disponible", lambda arxiv_id: False)
    monkeypatch.setattr(KnowledgeTool, "_fetch", lambda arxiv_id, base: b"%PDF-1.4 fake")
    monkeypatch.setattr(KnowledgeTool, "_pdf_a_markdown", lambda pdf: (_ for _ in ()).throw(KnowledgeError("markitdown no instalado")))
...
def test_refresh_from_objective_incluye_el_objetivo(context, monkeypatch):
    ...
    (context.root / "references" / "00-objetivo.md").write_text("# Objetivo\n\nPregunta: ¿qué tratamiento reduce el churn?\n", encoding="utf-8")
    monkeypatch.setattr(KnowledgeTool, "arxiv_search", lambda *a, **kw: [])

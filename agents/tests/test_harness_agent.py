...
def harness(context) -> HarnessAgent:
    ...
    (context.root / "harness" / "featureslist.json").write_text(json.dumps(BACKLOG, indent=2), encoding="utf-8")
...
def test_next_con_spec_no_sugiere_scope(harness):
    ...
    (harness.ctx.root / "references" / "00-objetivo.md").write_text("# Objetivo\n\n## Pregunta\n¿Q?\n", encoding="utf-8")
    result = harness.next()
    assert result.success
    assert result.data.get("sugerencia") != "plan scope"
...
def _rechazar(harness, veces: int) -> list:
    return [harness.record(agent="reviewer", id="A-001", verdict="rechazado", content=f"## Bloqueantes\nfalta el test {i}") for i in range(veces)]
...
def test_finish_rechaza_certeza_baja_explicita(harness):
    _write_gate(harness.ctx.root, GATE_OK)
    harness.start(id="A-001")
    result = harness.finish(id="A-001", evidence="pytest: 3 passed, 0 failed en 0.4s", certainty=0.4)
    assert not result.success
...
def test_finish_acepta_certeza_suficiente(harness):
    _write_gate(harness.ctx.root, GATE_OK)
    harness.start(id="A-001")
    result = harness.finish(id="A-001", evidence="pytest: 3 passed, 0 failed en 0.4s", certainty=0.9)
    assert result.success
...
def test_finish_hereda_la_certeza_baja_del_reviewer(harness):
    _write_gate(harness.ctx.root, GATE_OK)
    harness.start(id="A-001")
    harness.record(agent="reviewer", id="A-001", verdict="aprobado", content="## Criterios\ncumplidos", certainty=0.5)
...
def test_record_clampa_la_certeza_a_01(harness):
    harness.record(agent="reviewer", id="A-001", verdict="aprobado", content="ok", certainty=7.0)
    path = harness.ctx.root / "harness" / "progress" / "reviewer-A-001.md"
...
def test_record_con_packet_escribe_frontmatter(harness):
    result = harness.record(agent="implementer", id="A-001", content="## Qué hice\nsrc/model.py", packet=PACKET_OK)
    assert result.success
...
def test_record_con_packet_hereda_la_certeza_del_packet(harness):
    harness.record(agent="implementer", id="A-001", content="## Qué hice\nx", packet=PACKET_OK)
    path = harness.ctx.root / "harness" / "progress" / "implementer-A-001.md"
...
def test_record_rechaza_packet_con_ejes_desconocidos(harness):
    result = harness.record(agent="explorer", id="A-001", content="hola", packet='{"E":{},"μ":{"rol":"explorer"},"Zeta":1}')
    assert not result.success
    assert "desconocidos" in result.message


def test_record_rechaza_packet_sin_mu_rol(harness):
    result = harness.record(agent="explorer", id="A-001", content="hola", packet='{"E":{},"S":{}}')
    assert not result.success
    assert "μ" in result.message


def test_record_rechaza_cert_fuera_de_rango(harness):
    result = harness.record(agent="explorer", id="A-001", content="hola", packet='{"E":{},"μ":{"rol":"explorer","cert":1.7}}')
    assert not result.success

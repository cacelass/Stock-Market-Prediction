...
def _evaluar_caso(root: Path, caso: dict, top_k: int, hybrid: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "message": (
            f"top-{top_k}: {'ok' if acierto else 'FALLA'}" + (f" (pos {posicion})" if posicion else "") + f" — esperado {esperadas}, devuelto {fuentes[:3]}"
        ),
    }
...
def _agregar(casos: list[dict], top_k: int) -> dict[str, Any]:
    ...
    return {
        ...
        "lexical_share": round(sum(c.get("lexical_hits", 0) for c in casos) / devueltos, 3) if devueltos else 0.0,
        "top_k": top_k,
    }


def evaluate(root: Path, *, top_k: int = TOP_K, golden: list[dict] | None = None) -> dict[str, Any]:
    """
...
def suite(root: Path, *, top_k: int = TOP_K) -> dict[str, Any]:
    ...
    if not informe.get("available"):
        return {
            ...
            "failed": 0,
            "avg_duration_ms": 0,
            "results": [{"agent": "rag", "success": True, "message": f"no evaluado — {informe.get('reason')}"}],
        }

    filas = [{"agent": f"rag:{c['query'][:40]}", "success": c["success"], "message": c["message"]} for c in informe["cases"]]

#!/usr/bin/env bash
#
# init.sh — Puerta de entrada del arnés de Stock Market Prediction
#
...
[ "$JSON" -eq 1 ] || printf '\n\033[1m━━ init.sh · arnés de Stock Market Prediction ━━\033[0m\n'
...
PY=""
# El intérprete del .venv manda: el proyecto declara 3.12 y es el único con las
# dependencias instaladas (pytest, etc.). El python del sistema puede ser otra
# versión y sin dependencias; caer a él deja la puerta en rojo (QA-002).
for candidate in ".venv/bin/python" "python3.12" python3 python; do
...
if [ -z "$PY" ]; then
  ...
  if [ -z "$PY_VERSION" ]; then
    ...
    warn "python" "$PY es $PY_VERSION, el proyecto declara 3.12 — usa el intérprete de .venv"
  fi
fi
...
if [ -d "inversion" ]; then
  ok "paquete" "inversion/ presente"
else
  fail "paquete" "falta el paquete inversion/"
fi
